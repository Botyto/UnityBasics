...
public class BezierCurveEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if (showPoints)
		{
			...
			if (GUILayout.Button("Add Point"))
			{
				...
				pointObject.transform.position = new Vector3(SceneView.lastActiveSceneView.camera.transform.position.x, SceneView.lastActiveSceneView.camera.transform.position.y, curve.transform.position.z);
				BezierPoint newPoint = pointObject.AddComponent<BezierPoint>();

				newPoint.curve = curve;
				newPoint.handle1 = Vector3.left * 0.5f;
				...
				//pointObject.transform.SetSiblingIndex(curve.transform.childCount);
	...
	void DrawPointInspector(BezierPoint point, int index)
	{
		...
		if (index != 0 && GUILayout.Button(@"/\", GUILayout.Width(25)))
		{
			...
			//point.transform.SetSiblingIndex(point.transform.GetSiblingIndex() - 1);
		}
	...
	static void DrawPointSceneGUI(BezierPoint point)
	{
		...
		var fmh_200_64_638138695670877391 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position) * 0.1f, Vector3.zero, Handles.RectangleHandleCap);
		...
		if (point.handleStyle != BezierPoint.HandleStyle.None)
		{
			...
			var fmh_211_69_638138695670882372 = point.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(point.globalHandle1, HandleUtility.GetHandleSize(point.globalHandle1) * 0.075f, Vector3.zero, Handles.CircleHandleCap);
			...
			var fmh_219_69_638138695670884225 = point.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(point.globalHandle2, HandleUtility.GetHandleSize(point.globalHandle2) * 0.075f, Vector3.zero, Handles.CircleHandleCap);
